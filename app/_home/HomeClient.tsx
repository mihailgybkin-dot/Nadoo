...
export default function HomeClient() {
    ...
    (async () => {
      try {
        ...
        marker.current = new ymaps.Placemark(center, {}, {
          ...
          iconColor: '#22c55e', // ярко-зеленый кружок
        });
  ...
  // Подгрузка данных (топ в радиусе, иначе — ближайшие)
  useEffect(() => {
    let cancelled = false;
    ...
    (async () => {
      ...
      const inRadiusItems = (itemsData ?? []).filter(
        (i) => i.lat && i.lng && distance(center, [i.lat, i.lng]) <= RADIUS_METERS
      );
      const inRadiusTasks = (tasksData ?? []).filter(
        (t) => t.lat && t.lng && distance(center, [t.lat, t.lng]) <= RADIUS_METERS
      );
  ...
  return (
    <div>
      ...
      <div
        ref={mapRef}
        style={{ width: '100%', height: 360, border: '1px solid #eee', borderRadius: 8 }}
      />
...
function Cards({ list }: {
  list: { id: string; title: string; price?: number; address: string; image?: string | null; href: string }[];
}) {
...
async function loadYandex() {
  ...
  if (window.ymaps) {
    return new Promise<any>((resolve) => window.ymaps!.ready(() => resolve(window.ymaps)));
  }
